...
<template>
  ...
  <div class="tui-3d-icon-apart-wrap">
    <p class="desc">每个图标均为独立的T3DPanel组件，配置参数独立定义</p>
    <div class="img-wrap">
    ...
    <div class="icon-grid">
      <div class="grid-item" v-for="(item,index) in state.gridData" :key="index">
        <Tui3DPanel :config="t3DGridConfig(item,index)" :index="index">
          <template #default="parmas">
            <div class="grid-item-box">
              <i :class="`grid-icon ${item.icon} ${parmas?.boxShadowName}`" :style="`--iconBGD:#${state.colorsN[index]};--iconBGL:#${state.colorsL[index]}`"></i>
...
<style lang="less">
.tui-3d-icon-apart-wrap{ padding:60px 40px 0 40px; overflow: auto; .poa; inset:0;
  .desc{text-align: center;}
  .img-wrap{ width: 100%; height:100px; margin:20px auto 40px auto;
    img{width: 100%; height: 100%; .bdra(10px);}
  }
