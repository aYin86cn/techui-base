<template>
<TuiEcharts :chartOption="chartOption" @chartClick="chartClick"></TuiEcharts>
</template>
<script>
...
data() {
  ...
  watch:{
    "$tState.mapData.name"(val1,val2){
      if(this.chartOption.series.length==0){ return; }
      let drNeg=this.$tService.randomData({
        min:-100,
        max:0,
        length:6,
        group:1
      })
      let drPos=this.$tService.randomData({
        min:100,
        max:200,
        length:6,
        group:1
  ...
  methods: {
    ...
    processData(){
      let {colors,data}=this.chartData, processedData=[],legend=[],yAxis=[]
      for(let i=1;i<=2;i++){
        legend.push(this.$t(`dashboardA.l1.legend${i}`))
      }
      for(let i=1;i<=6;i++){
        yAxis.push(this.$t(`dashboardA.l1.axis`)+i)
      }
      yAxis.push
